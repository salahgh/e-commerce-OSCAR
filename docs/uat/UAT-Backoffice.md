...
|----|-------|---------------|--------|-----------------|------------------|----------|--------|
| BO-AUTH-01 | Connexion avec identifiants valides | Compte admin existant | 1. Ouvrir `/login` 2. Saisir identifiant + mot de passe 3. Cliquer « Se connecter » | `superadmin` / `superadmin123` | Redirection vers le tableau de bord, session ouverte | Haute | ☐ |
| BO-AUTH-02 | Connexion — mot de passe erroné | — | 1. Saisir identifiant valide + mot de passe faux 2. Valider | mdp `xxxx` | Message d'erreur « identifiants invalides », pas de redirection | Haute | ☐ |
| BO-AUTH-03 | Connexion — identifiant inexistant | — | 1. Saisir un identifiant inconnu + mdp 2. Valider | `inconnu` | Message d'erreur, reste sur `/login` | Haute | ☐ |
| BO-AUTH-04 | Champs obligatoires | — | 1. Laisser identifiant et/ou mdp vides 2. Valider | champs vides | Validation : champs requis signalés, soumission bloquée | Moyenne | ☐ |
| BO-AUTH-05 | Mot de passe trop court | — | 1. Saisir mdp < 4 caractères 2. Valider | `ab` | Erreur de validation (min 4 caractères) | Basse | ☐ |
| BO-AUTH-06 | Option « Se souvenir de moi » | — | 1. Cocher « Se souvenir » 2. Se connecter 3. Fermer puis rouvrir le navigateur | — | Session conservée après réouverture | Moyenne | ☐ |
| BO-AUTH-07 | Persistance de session au rafraîchissement | Connecté | 1. Rafraîchir la page (F5) | — | Reste connecté, contexte restauré (requête `Me`) | Haute | ☐ |
| BO-AUTH-08 | Déconnexion | Connecté | 1. Ouvrir le menu utilisateur 2. Cliquer « Déconnexion » | — | Retour à `/login`, état Redux et localStorage purgés | Haute | ☐ |
| BO-AUTH-09 | Accès à une route protégée sans authentification | Déconnecté | 1. Saisir une URL protégée (ex. `/products`) | — | Redirection automatique vers `/login` | Haute | ☐ |
| BO-AUTH-10 | Accès refusé par permission manquante | Connecté avec rôle limité (sans `ReadOrder`) | 1. Tenter d'accéder à `/orders` | rôle limité | Redirection vers `/access-denied` | Haute | ☐ |
| BO-AUTH-11 | Page « Accès refusé » | idem | 1. Atteindre `/access-denied` | — | Message explicite, lien retour | Moyenne | ☐ |
| BO-AUTH-12 | Masquage des éléments de menu selon permissions | Rôle limité | 1. Se connecter 2. Observer le menu latéral | — | Seuls les modules autorisés sont visibles | Haute | ☐ |
| BO-AUTH-13 | Désactivation des actions selon permissions | Rôle sans `DeleteCatalog` | 1. Ouvrir la liste produits | — | Bouton « Supprimer » masqué/désactivé (avec infobulle) | Haute | ☐ |
| BO-AUTH-14 | Permission OR (anyOf) | Rôle avec `ReadOrder` uniquement | 1. Accéder au tableau de bord (autorisé si `ReadCatalog` OU `ReadOrder` OU `ReadCustomer`) | — | Accès accordé | Moyenne | ☐ |
| BO-AUTH-15 | Permission AND (allOf) | Rôle ne possédant pas toutes les permissions requises | 1. Accéder à une page exigeant plusieurs permissions | — | Accès refusé tant que toutes ne sont pas présentes | Moyenne | ☐ |
| BO-AUTH-16 | Super Admin — accès total | Super Admin | 1. Parcourir tous les modules | — | Aucun blocage, tout est accessible | Haute | ☐ |

## 2. Tableau de bord & Rapports — `BO-DASH`

| ID | Titre | Préconditions | Étapes | Données de test | Résultat attendu | Priorité | Statut |
|----|-------|---------------|--------|-----------------|------------------|----------|--------|
| BO-DASH-01 | Chargement du tableau de bord | Connecté, données présentes | 1. Ouvrir `/` | — | KPIs, graphiques, commandes récentes et alertes stock s'affichent | Haute | ☐ |
| BO-DASH-02 | Cartes KPI | idem | 1. Vérifier chaque carte | — | Ventes totales, nb commandes, panier moyen, taux de conversion, nouveaux clients, rétention affichés et cohérents | Haute | ☐ |
| BO-DASH-03 | Sélecteur de période (7/30/90 j) | idem | 1. Changer la période | 7j → 30j → 90j | KPIs et graphiques se recalculent | Moyenne | ☐ |
...
| ID | Titre | Préconditions | Étapes | Données de test | Résultat attendu | Priorité | Statut |
|----|-------|---------------|--------|-----------------|------------------|----------|--------|
| BO-PROD-40 | Édition d'un produit existant | Produit existant | 1. `/products/:id/edit` 2. Modifier des champs 3. Enregistrer | — | Données pré-remplies, modifications sauvegardées | Haute | ☐ |
| BO-PROD-41 | Consultation (lecture seule) | — | 1. Ouvrir `/products/:id` | — | Détails, galerie, variantes, collections affichés | Moyenne | ☐ |
| BO-PROD-42 | Suppression avec confirmation | — | 1. Cliquer « Supprimer » 2. Confirmer | — | Dialogue de confirmation puis suppression + toast | Haute | ☐ |
| BO-PROD-43 | Annulation de la suppression | — | 1. Ouvrir le dialogue 2. Annuler | — | Produit conservé | Moyenne | ☐ |
