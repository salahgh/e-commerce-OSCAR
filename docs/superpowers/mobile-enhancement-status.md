...
**Health:** 69 unit tests pass; `npm run lint` = 0 errors (warnings are pre-existing). M0/M1a/M1b/M1c source is `tsc`-clean (baseline still 155 pre-existing errors; **zero new** introduced).

## Next up (recommended order)
1. **Reachable CIB/Baridimob payment screens** (flow + deep-link return). NOTE: the actual gateway *initiate* call is backend-blocked, so this is a partial slice until backend work happens.
2. **M2 — Engagement/retention** (reviews, push notifications, recently-viewed, wishlist sync) — mostly backend-dependent, largely deferred under mobile-only.
3. **M3 — UX polish / accessibility / real dark mode / Arabic fonts / haptics.**
4. **M4 — Native capabilities & release hardening** (deep links, EAS OTA, biometric, camera/media, typed `app.config.ts` + endpoint hardening).
