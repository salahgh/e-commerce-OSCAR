      ...
      <TooltipPrimitive.Root>
        ...
        <TooltipPrimitive.Portal>
          <TooltipPrimitive.Content
            ...
            className={cn(
              'z-[9999]',
              'bg-popover text-popover-foreground text-sm rounded-lg shadow-2xl',
  ...
  return (
    <div className="p-5 w-[700px]">
      ...
      <div className="flex gap-5 mb-5">
        <div className="flex-shrink-0">
          {product.featuredAsset?.preview ? (
            <img
              ...
              alt={product.name}
              className="w-36 h-36 rounded-xl object-cover border-2 border-border shadow-lg"
            />
          ) : (
            <div className="w-36 h-36 rounded-xl bg-muted flex items-center justify-center border-2 border-border">
            ...
            <div className="flex gap-1.5 mt-2 justify-center">
              {product.assets.slice(0, 5).map((asset, idx) => (
                <img
                  ...
                  src={asset.preview}
                  alt={`Image ${idx + 1}`}
                  className="w-7 h-7 rounded object-cover border border-border opacity-70 hover:opacity-100 transition-opacity"
                />
              ))}
              {product.assets.length > 5 && (
                <div className="w-7 h-7 rounded bg-muted flex items-center justify-center text-[9px] text-muted-foreground">
                  +{product.assets.length - 5}
                </div>
              )}
            </div>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2 mb-1">
            ...
            <span className="text-muted-foreground text-[10px] font-mono bg-card px-1.5 py-0.5 rounded flex-shrink-0">
              #{product.id}
            </span>
          </div>
          <p className="text-muted-foreground text-xs mb-2 font-mono">{product.slug}</p>
      ...
      <div className="grid grid-cols-4 gap-2 mb-3">
        ...
        <div className="text-center p-2 bg-card rounded-lg">
          <div className="text-[9px] text-muted-foreground uppercase">Alloué</div>
      ...
      {product.variants && product.variants.length > 0 && (
        <div className="mb-3">
          ...
          <div className="bg-card/50 rounded-lg border border-border/50 overflow-hidden">
            <div className="grid grid-cols-12 gap-1 px-2 py-1.5 bg-card text-[9px] text-muted-foreground uppercase font-semibold">
            ...
            {product.variants.length > 8 && (
              <div className="text-center py-1.5 text-[10px] text-muted-foreground bg-card/50">
