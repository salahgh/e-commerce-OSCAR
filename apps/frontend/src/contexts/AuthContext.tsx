...
import {
  ...
  // NOTE: generated by `pnpm --filter @oscar/graphql-shop codegen` after the new
  // UpdateCustomerAvatar operation was added to shop-auth.graphql.
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const uploadAvatar = async (file: File) => {
    ...
    if (!data?.updateCustomerAvatar) throw new Error('Avatar upload failed - no response');
    // Refetch the full customer so the avatar (and any other fields) stay in sync.
    await refetchCustomer();
  };
